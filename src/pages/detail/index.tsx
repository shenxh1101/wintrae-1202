...
const DetailPage: React.FC = () => {
  ...
  const { toggleFavorite, isFavorite: checkFavorite } = usePosts();

  useEffect(() => {
    const postId = getQueryString('id');
    console.log('[Detail] 获取帖子ID:', postId);
    ...
    setTimeout(() => {
      ...
      setLoading(false);
      if (found) {
        setIsFavorite(checkFavorite(found.id));
      }
    }, 300);
  }, [checkFavorite]);
  ...
  }, [post, isFavorite, toggleFavorite]);
  ...
  const handleClaim = useCallback(() => {
    ...
    Taro.showModal({
      title: '确认认领',
      content: `确认要认领「${post.title}」吗？请确保这是您的物品。`,
      success: (res) => {
        if (res.confirm) {
  ...
  const handleImagePreview = useCallback((current: string) => {
    if (!post) return;
    Taro.previewImage({
      current,
      urls: post.images,
    ...
    return (
      <View className={styles.page}>
        <View style={{ padding: 100 }}>
          <Text style={{ textAlign: 'center', display: 'block', color: '#86909C' }}>帖子不存在或已删除</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    <View className={styles.page}>
          ...
          <View className={styles.authorInfo}>
            ...
            <View className={styles.authorDetail}>
              ...
              <Text className={styles.authorMeta}>
                {post.author.role === 'student' ? '学生' : '后勤人员'} · {formatRelativeTime(post.createdAt)}
              </Text>
            </View>
          </View>
          <View className={styles.tags}>
            ...
            {post.status !== 'active' && (
              <Tag type={`status-${post.status}` as any}>
                {post.status === 'active' ? '进行中' : post.status === 'claimed' ? '已认领' : '已关闭'}
              </Tag>
            )}
          </View>
        </View>
      ...
      <View className={styles.actionBar}>
        <Button
          className={classnames(styles.iconBtn, isFavorite && styles.active)}
